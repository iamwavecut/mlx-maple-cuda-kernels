...
def _make_cuda_router_kernel(profile):
    ...
    source = """
        ...
        if (tid == 0) {
            unsigned int* counter = (unsigned int*)ctr_in;
        ...
        __syncthreads();
        if (tid < 8) {
            out_indices[tid] = selected_indices[tid];
            out_scores[tid] = selected_exp[tid] / selected_sum;
        }
        __syncthreads();
        if (tid == 0) {
            atomicExch((unsigned int*)ctr_in, 0u);
            __threadfence();
        }
    """
    return mx.fast.cuda_kernel(
        name=f"maple_fused_router_{profile.name}",
        input_names=["x", "w", "ctr_in"],
        output_names=["out_indices", "out_scores", "logits_scratch"],
        source=source,
    )
...
class MapleGate(nn.Module):
    ...
    def _probe(self, x):
        ...
        profile = _cuda_profile() if _kernel_backend() == "cuda" else None
        if profile is not None and profile.router_reference_gemv:
            return (
                valid
                and bool(mx.array_equal(inds, ref_inds))
                and bool(mx.allclose(scores, ref_scores, atol=1e-5))
            )
        return valid and bool(
            mx.allclose(mx.sort(scores), mx.sort(ref_scores), atol=1e-5)
        )
