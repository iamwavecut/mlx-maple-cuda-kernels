...
import argparse, hashlib, inspect, json, statistics
...
def main():
    ...
    records=[{
        ...
        'prompt_sha256':hashlib.sha256(','.join(map(str,prompt)).encode()).hexdigest(),
    }]
    expected=None
    for mode in MODES:
        configure(model,mode); result=run(model,tokenizer,prompt,args.generation_tokens)
    ...
    for block in range(1,args.blocks+1):
        ...
        if (block-1)//len(ORDERS) % 2: order=tuple(reversed(order))
        for position,mode in enumerate(order,1):
            configure(model,mode); result=run(model,tokenizer,prompt,args.generation_tokens)
            if result['token_sha256'] != expected:
                raise RuntimeError(f"token mismatch in block {block} mode {mode}")
            qk,lhs=MODES[mode]
            record={'type':'trial','block':block,'position':position,'mode':mode,
                    'qk':qk,'lhs':lhs,**result}
    ...
    for mode in MODES:
        ...
        records.append({'type':'summary','mode':mode,
