...
Warm `B=1`, `L=1` BF16 decode, 128-token prompt / 512-token generation,
medians over fresh interleaved processes. `off` is portable MLX with every
fusion disabled; `strict` adds the two array-exact fusions; `exact MK` is
the exact MoE megakernel; `+attn MK` adds the attention megakernel — the
shipped default is the last populated column of each row. Each row is one
device instance on one host; for a host-bound workload the CPU moves the
number more than the GPU does, so compare within rows, not across them.

| GPU | CC | off | strict | exact MK | + attn MK | stream vs stock |
| --- | --- | ---: | ---: | ---: | ---: | --- |
...
protocol; `exact MK` from the exact-default campaign. Full records:
[`results/cuda/`](results/cuda/).

Lane defaults and their exactness status:
...
- detailed allowlisted artifacts under
  ...
  the fusion-era campaigns in
  [`sm86-fusion-release.jsonl`](results/cuda/sm86-fusion-release.jsonl) and
  [`fusion-multiarch.jsonl`](results/cuda/fusion-multiarch.jsonl); the
  megakernel and attention evidence log is
